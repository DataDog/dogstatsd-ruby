...
describe Datadog::Statsd::OriginDetection do
  ...
  it 'writes a file in the fake filesystem' do
    read = nil
    FakeFS.with_fresh do
      FileUtils.mkdir_p('/tmp')
      File.write('/tmp/test.txt', 'hello')
      read = File.read('/tmp/test.txt')
    end

    expect(read).to eq('hello')
  end 

  it 'extracts the container ID from the cgroup file' do
    container_id = nil
    FakeFS.with_fresh do
  ...
  describe 'read container id' do
    [
      ...
      it "extracts container ID from case #{index + 1}" do
        result = nil
  ...
  it 'extracts container ID from mountinfo' do
    container_id = nil
    FakeFS.with_fresh do
      # Set up fake /proc/self directory
      FileUtils.mkdir_p('/proc/self')
...
2036 2213 0:256 / /sys/firmware ro,relatime - tmpfs tmpfs ro
    ...
    test_cases.each_with_index do |test_case, index|
      it "extracts container ID from mountinfo case #{index + 1}" do
        result = ""
        FakeFS.with_fresh do
          FileUtils.mkdir_p('/proc/self')
  ...
  describe '#parse_cgroup_node_path' do
    test_cases = [
      {
  ...
  describe '#get_cgroup_inode' do
    test_cases = [
      {
        ...
        proc_self_cgroup_content: "0::/system.slice/docker-abcdef0123456789abcdef0123456789.scope\n",
        controller: nil,
      ...
      },
      {
        ...
        cgroup_node_dir: 'system.slice/docker-abcdef0123456789abcdef0123456789.scope',
        proc_self_cgroup_content: 'nothing',
        controller: nil,
        expected_result: nil
      }
    ]
    ...
    test_cases.each_with_index do |test_case, index|
      it "handles: #{test_case[:description]}" do
        result = ""
        FakeFS.with_fresh do
          FileUtils.mkdir_p('/proc/self')
          File.write('/proc/self/cgroup', test_case[:proc_self_cgroup_content])

          controller = test_case[:controller]
          segments = ['/sys/fs/cgroup', controller, test_case[:cgroup_node_dir]].compact
          cgroup_node_path = File.join(*segments)
          FileUtils.mkdir_p(File.dirname(cgroup_node_path))
          File.write(cgroup_node_path, '')
          inode = File.stat(cgroup_node_path).ino
          unless test_case[:expected_result].nil?
    ...
    test_cases = [
      {
        ...
        proc_self_cgroup_content: "4:blkio:/kubepods/burstable/podfd52ef25-a87d-11e9-9423-0800271a638e/8c046cb0b72cd4c99f51b5591cd5b095967f58ee003710a45280c28ee1a9c7fa\n",
        is_host_cgroup_ns: true,
        expected_result: '8c046cb0b72cd4c99f51b5591cd5b095967f58ee003710a45280c28ee1a9c7fa'
      },
      {
      ...
      it "handles: #{test_case[:description]}" do
        ...
        FakeFS.with_fresh do
          ...
          if test_case[:cgroup_node_dir]
            cgroup_path = File.join('/sys/fs/cgroup', test_case[:cgroup_node_dir])
            FileUtils.mkdir_p(File.dirname(cgroup_path))
            File.write(cgroup_path, '')

            inode = File.stat(cgroup_path).ino
            unless test_case[:expected_result].nil?
              test_case[:expected_result] = test_case[:expected_result].gsub("{inode}", inode.to_s)
            end
          end
